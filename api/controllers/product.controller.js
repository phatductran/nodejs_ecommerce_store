        ...
        if (category) {
          ...
          if (subcategoryList.length > 0) {
            ...
            for (let j = 0; j< subcategoryList.length; j++){
              const products = await ProductObject.getProductsBy({subcategoryId: subcategoryList[j].id})
  ...
  showProductsBySubcategoryId: async (req, res) => {
    try {
      if (req.query.subcategoryId) {
        const subcategory = await SubcategoryObject.getOneSubcategoryBy({ _id: req.query.subcategoryId })
        if (subcategory) {
          const products = await ProductObject.getProductsBy({subcategoryId: subcategory.id})
          if (products.length > 0 ) {
